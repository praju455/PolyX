...
<<<<<<< HEAD
    ...
    if (postId) {
      ...
      // Parse @mentions and add notifications
      const mentions = extractMentions(parsed.data.text);
      for (const handle of mentions) {
        try {
          const profile = await getProfileByHandle(handle);
          if (profile && profile.owner.toLowerCase() !== parsed.data.user.toLowerCase()) {
            addMentionNotification(profile.owner, parsed.data.user, postId, Date.now()).catch(() => {});
          }
        } catch {}
      }
    }
...
app.get("/api/feed", async (req, res) => {
  try {
    ...
    if (tag && tag.length > 0) {
      postIds = await getPostIdsByHashtag(tag);
      if (postIds.length === 0) {
        return respond<Post[]>(res, { success: true, data: [] });
      }
    }
    const nextId = await fetchPostCount();
    const allPostIds: number[] = postIds || [];
    if (!postIds) {
...
<<<<<<< HEAD
    ...
    if (query.length === 42 && query.startsWith("0x")) {
      try {
        const profile = await getProfileByOwner(query);
        if (profile) {
          return respond<Profile[]>(res, { success: true, data: [profile] });
        }
      } catch {}
...
app.get("/api/search/posts", async (req, res) => {
  try {
    ...
    const postIds = await searchPosts(query);
    if (postIds.length === 0) {
      return respond<Post[]>(res, { success: true, data: [] });
    }
    const posts = await readBatch(postIds);
    const filtered = posts.filter((p) => !p.deleted);
    filtered.sort((a, b) => b.timestamp - a.timestamp);
    return respond<Post[]>(res, { success: true, data: filtered });
  } catch (err: any) {
    return respond(res, { success: false, error: err.message || "Failed to search posts" });
  }
});
...
app.get("/api/hashtags/trending", async (req, res) => {
  try {
    ...
    return respond(res, { success: true, data: trending });
  } catch (err: any) {
    return respond(res, { success: false, error: err.message || "Failed to fetch trending hashtags" });
  }
});

// Bookmarks
app.post("/api/bookmark", async (req, res) => {
  const parsed = z.object({ user: address, postId: z.number().int().positive() }).safeParse(req.body);
  if (!parsed.success) return respond(res, { success: false, error: parsed.error.message });
  try {
    await addBookmark(parsed.data.user, parsed.data.postId);
    return respond(res, { success: true, data: { bookmarked: true } });
  } catch (err: any) {
    return respond(res, { success: false, error: err.message || "Failed to bookmark" });
  }
});
...
app.delete("/api/bookmark", async (req, res) => {
  ...
  if (!parsed.success) return respond(res, { success: false, error: parsed.error.message });
  try {
    await removeBookmark(parsed.data.user, parsed.data.postId);
    return respond(res, { success: true, data: { bookmarked: false } });
  } catch (err: any) {
    return respond(res, { success: false, error: err.message || "Failed to remove bookmark" });
  }
});

app.get("/api/bookmarks/:user", async (req, res) => {
  try {
    const user = req.params.user;
    if (!user || user.length !== 42 || !user.startsWith("0x")) {
      return respond(res, { success: false, error: "Invalid user address" });
    }
    const postIds = await getBookmarks(user);
    if (postIds.length === 0) {
      return respond<Post[]>(res, { success: true, data: [] });
    }
    const posts = await readBatch(postIds);
    ...
    return respond(res, { success: false, error: err.message || "Failed to fetch bookmarks" });
  }
});

app.get("/api/post/:id/bookmarked/:user", async (req, res) => {
  try {
    const postId = parseInt(req.params.id, 10);
    const user = req.params.user;
    if (isNaN(postId) || postId < 1) {
    ...
    const bookmarked = await isBookmarked(user, postId);
    return respond(res, { success: true, data: { bookmarked } });
  } catch (err: any) {
    return respond(res, { success: false, error: err.message || "Failed to check bookmark status" });
  }
});

// Reactions
const reactionSchema = z.object({ user: address, postId: z.number().int().positive(), reactionType: z.enum(["fire", "love", "laugh"]) });
app.post("/api/reaction", async (req, res) => {
  const parsed = reactionSchema.safeParse(req.body);
  if (!parsed.success) return respond(res, { success: false, error: parsed.error.message });
  try {
    await addReaction(parsed.data.user, parsed.data.postId, parsed.data.reactionType);
    return respond(res, { success: true, data: { added: true } });
  } catch (err: any) {
    return respond(res, { success: false, error: err.message || "Failed to add reaction" });
  }
});

app.delete("/api/reaction", async (req, res) => {
  const parsed = z.object({ user: address, postId: z.number().int().positive() }).safeParse(req.body);
  if (!parsed.success) return respond(res, { success: false, error: parsed.error.message });
  try {
    await removeReaction(parsed.data.user, parsed.data.postId);
    return respond(res, { success: true, data: { removed: true } });
  } catch (err: any) {
    return respond(res, { success: false, error: err.message || "Failed to remove reaction" });
  }
});

app.get("/api/post/:id/reactions", async (req, res) => {
  try {
    const postId = parseInt(req.params.id, 10);
    if (isNaN(postId) || postId < 1) {
      return respond(res, { success: false, error: "Invalid post ID" });
    ...
    return respond(res, { success: true, data: reactions });
  } catch (err: any) {
    return respond(res, { success: false, error: err.message || "Failed to fetch reactions" });
  }
});

app.get("/api/post/:id/reaction/:user", async (req, res) => {
  try {
    const postId = parseInt(req.params.id, 10);
    const user = req.params.user;
    if (isNaN(postId) || postId < 1) {
    ...
    const reaction = await getUserReaction(user, postId);
    return respond(res, { success: true, data: { reactionType: reaction } });
  } catch (err: any) {
    return respond(res, { success: false, error: err.message || "Failed to fetch user reaction" });
  }
});
...
<<<<<<< HEAD
    ...
    try {
      ...
      for (const m of mentionNotifs) {
        notifications.push({ type: "mention", from: m.from, postId: m.postId, timestamp: m.timestamp });
      }
    } catch {}

...
export function PostCard({ post, showComments = false, isComment = false }: { post: Post; showComments?: boolean; isComment?: boolean }) {
  ...
  const { data: reactions } = useQuery<Record<string, number>>({
    queryKey: ["reactions", post.id],
    queryFn: () => api.getReactions(post.id),
    enabled: !!post.id,
  });

  const { data: userReaction } = useQuery<{ reactionType: string | null }>({
    queryKey: ["userReaction", post.id, address],
    queryFn: () => (address ? api.getUserReaction(post.id, address) : Promise.resolve({ reactionType: null })),
    enabled: !!address && !!post.id,
  });
  ...
  const bookmarkMutation = useMutation({
    mutationFn: async () => {
      if (!address) throw new Error("Connect wallet first");
      if (isBookmarked) {
        return api.unbookmark(address, post.id);
      } else {
        return api.bookmark(address, post.id);
      }
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["bookmarkStatus", post.id, address] });
      queryClient.invalidateQueries({ queryKey: ["bookmarks", address] });
    },
  });

  const reactionMutation = useMutation({
    mutationFn: async ({ type }: { type: "fire" | "love" | "laugh" }) => {
      if (!address) throw new Error("Connect wallet first");
      if (userReaction?.reactionType === type) {
        return api.removeReaction(address, post.id);
      }
      return api.addReaction(address, post.id, type);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["reactions", post.id] });
      queryClient.invalidateQueries({ queryKey: ["userReaction", post.id, address] });
    },
  });
        ...
        <div className="flex items-center gap-1 group/reactions">
          {(["fire", "love", "laugh"] as const).map((type) => (
            <button
              ...
              disabled={reactionMutation.isPending || !address}
          ...
          {(reactions?.fire || reactions?.love || reactions?.laugh) ? (
            <span className="text-xs opacity-60 ml-1">
              {[reactions?.fire || 0, reactions?.love || 0, reactions?.laugh || 0].filter(Boolean).join(" ")}
            </span>
          ) : null}
        </div>
        <button
